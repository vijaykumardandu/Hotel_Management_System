...
class HotelManagement:
        ...
        if name and phone:
            ...
            for g in self.guests:
                if g["Name"].lower() == name.lower():
                    messagebox.showwarning("Already Checked In", f"{name} is already checked in.")
                    return
    ...
    def check_out(self):
        name = self.name_var.get().strip()
        if not name:
            messagebox.showwarning("Input Required", "Please enter the guest's name to check out.")
            return
